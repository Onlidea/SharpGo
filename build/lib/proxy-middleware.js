...
var ua = '';
var Log = require('../log');
var log = Log('proxy-middleware');

module.exports = function(options) {

  log.debug('-----: ', os.tmpdir() + '/mobileProxyCookie');
  fs.readFile(os.tmpdir() + '/mobileProxyCookie', function(err, data) {
    if (err) {
      log.debug('Not found cookie persistence.');
      return;
    }
  ...
  return function(req, resp, next) {
    ...
    var myReq = request(opts, function(myRes) {
      ...
      if (myRes.headers['set-cookie']) {
        ...
        log.debug('Persist cookies to file.');
      }
...
function matchReqByConfig(req) {
  ...
  if (matchedConfig) {
    log.debug('url ', req.url, ' matched: ', matchedConfig.pattern, ' method: ', matchedConfig.method);
